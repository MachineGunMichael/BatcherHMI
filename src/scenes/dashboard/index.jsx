...
const Dashboard = () => {
  ...
  // Toggle visibility of a series with context state
  ...
  const throughputData = useMemo(() => {
    const series = Object.entries(throughput)
      .filter(([prog]) => dashboardVisibleSeries[prog])
      .map(([prog, arr]) => ({
        id: prog,
        color: colorMap[prog],
    ...
    if (dashboardVisibleSeries["Total"]) {
      series.push({
        id: "Total",
    ...
    return series;
  }, [throughput, throughput_sum, windowTimestamps, index, colorMap, dashboardVisibleSeries]);
  ...
  const giveawayData = useMemo(() =>
    ...
    [giveaway, windowTimestamps, index, colorMap, dashboardVisibleSeries]
  );

  const rejectsData = useMemo(() => {
    if (!dashboardVisibleSeries["Total"]) return [];
  ...
  }, [rejects, windowTimestamps, index, colorMap, dashboardVisibleSeries]);

  // Filter pie chart data
  const batchPerMinData = useMemo(
    () =>
      pie_batch_per_min[index]
        .filter(d => dashboardVisibleSeries[d.program])
        .map(d => ({
          id: d.program,
          value: d.value,
          color: colorMap[d.program],
        })),
    [pie_batch_per_min, index, colorMap, dashboardVisibleSeries]
  );
  ...
  const batchTotalData = useMemo(
    ...
    [pie_batch_total, index, colorMap, dashboardVisibleSeries]
  );
  ...
  const giveawayPercentData = useMemo(
    ...
    [pie_giveaway_percent, index, colorMap, dashboardVisibleSeries]
  );
  ...
  const giveawayGramData = useMemo(
    ...
    [pie_giveaway_gram_batch, index, colorMap, dashboardVisibleSeries]
  );
        ...
        >
          {Object.entries(colorMap).map(([program, color]) => (
            <Box 
              ...
              sx={{ 
                cursor: 'pointer',
                opacity: dashboardVisibleSeries[program] ? 1 : 0.4,
                transition: 'all 0.2s',
                '&:hover': {
                  transform: 'scale(1.05)',
                },
                border: dashboardVisibleSeries[program] ? 'none' : `1px solid ${colors.grey[300]}`,
                borderRadius: '4px',
                padding: '2px 6px',
              }}
            >
